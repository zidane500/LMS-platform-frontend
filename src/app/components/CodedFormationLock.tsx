        ...
        {nbTotal > 0 && (
          <div className="bg-gray-50 dark:bg-slate-800/60 border border-gray-200 dark:border-slate-700/50 rounded-2xl p-4 space-y-3">
            ...
            {tousCertifies ? (
              ...
              /* ✅ Fix 2 — 100% progression mais certificats non encore générés */
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 rounded-xl p-3 text-center"
              >
                <p className="text-blue-700 dark:text-blue-400 text-sm font-semibold">
                  🏁 Toutes les formations sont terminées à 100% !
                </p>
                <p className="text-blue-600/70 dark:text-blue-500/70 text-xs mt-0.5">
                  Rendez-vous sur la page Certificats pour générer vos
                  certificats et recevoir le code d'accès automatiquement.
                </p>
                <button
                  onClick={() => navigate("/app/certificates")}
                  className="mt-2 text-xs px-3 py-1.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors font-semibold"
                >
                  🎓 Générer mes certificats
                </button>
              </motion.div>
            ) : (
