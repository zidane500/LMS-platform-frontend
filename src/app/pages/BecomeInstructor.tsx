...
import { ArrowLeft, Upload, Award, X, FileText, Clock, CheckCircle, XCircle, Loader2 } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { submitInstructorRequest, getMyRequest } from '../services/demandeService';
...
export const BecomeInstructor: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    specialty: '', experience: 0, motivation: '', languages: '',
  });
  ...
  if (existingRequest && existingRequest.status !== 'rejected') {
    ...
    const config = statusConfig[existingRequest.status as 'pending' | 'accepted'];

    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-purple-50/30">
        <div className="max-w-2xl mx-auto p-6 space-y-6">
          <Button variant="ghost" onClick={() => navigate('/app/profile')} className="gap-2">
          ...
          >
            <Card>
              <CardContent className="p-12 text-center space-y-5">
                ...
                {/* Badge statut */}
                <span className={`inline-block px-5 py-2 rounded-full text-sm font-semibold ${config.color}`}>
                  {config.label}
                </span>

                {/* Description */}
                <p className="text-gray-600 max-w-md mx-auto">{config.desc}</p>

                {/* Détails de la demande */}
                <div className="mt-4 text-left bg-gray-50 rounded-xl p-4 space-y-2 text-sm">
                  <p><span className="font-medium text-gray-600">Spécialité :</span> {existingRequest.specialty}</p>
                  <p><span className="font-medium text-gray-600">Expérience :</span> {existingRequest.experience} ans</p>
                  <p><span className="font-medium text-gray-600">Langues :</span> {existingRequest.languages?.join(', ')}</p>
                  {existingRequest.createdAt && (
                    <p><span className="font-medium text-gray-600">Soumise le :</span> {new Date(existingRequest.createdAt).toLocaleDateString('fr-FR')}</p>
                  )}
                </div>
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, type: 'cv' | 'certificate') => {
    ...
    if (tooBig) {
      toast.error(`"${tooBig.name}" dépasse 5 MB`); e.target.value = ''; return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (cvFiles.length === 0)          { toast.error('Veuillez télécharger au moins un CV (PDF)');          return; }
    if (certificateFiles.length === 0) { toast.error('Veuillez télécharger au moins une attestation (PDF)'); return; }
    setLoading(true);
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        {existingRequest?.status === 'rejected' && (
          ...
          >
            ...
            <div>
              <p className="font-semibold text-red-800">Votre précédente demande a été refusée.</p>
              <p className="text-red-700 text-sm mt-1">
                Vous pouvez soumettre une nouvelle candidature en remplissant le formulaire ci-dessous.
              </p>
            </div>
          </motion.div>
        )}
        ...
        <div className="text-center space-y-4 mb-8">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">
            Partagez votre expertise. Remplissez ce formulaire pour soumettre votre candidature.
          </p>
        </div>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <Input value={formData.specialty} onChange={e => handleChange('specialty', e.target.value)} placeholder="Ex: Développement Web, Data Science" required />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label>Expérience (années) *</Label>
                  <Input type="number" min="0" value={formData.experience || ''} onChange={e => handleChange('experience', parseInt(e.target.value) || 0)} required />
                </div>
                <div className="space-y-2">
                  <Label>Langue(s) enseignée(s) *</Label>
                  <Input value={formData.languages} onChange={e => handleChange('languages', e.target.value)} placeholder="Français, Anglais (séparées par virgules)" required />
                </div>
              </div>

              <div className="space-y-2">
                <Label>Motivation *</Label>
                <Textarea value={formData.motivation} onChange={e => handleChange('motivation', e.target.value)} rows={5} placeholder="Expliquez pourquoi vous souhaitez devenir formateur..." required />
              </div>
  ...
  <p className="text-sm text-gray-600">Cliquez pour ajouter un ou plusieurs CV</p>
  <p className="text-xs text-gray-400 mt-1">PDF uniquement, max 5MB par fichier</p>
  <input type="file" accept=".pdf" multiple className="hidden" onChange={e => handleFileChange(e, 'cv')} />
</label>
{cvFiles.length > 0 && (
  <div className="space-y-2 mt-2">
    {cvFiles.map((file, index) => (
      <div key={index} className="border-2 border-green-200 bg-green-50 rounded-lg p-3 flex items-center gap-3">
        ...
        <button
          type="button"
          onClick={() => setCvFiles(prev => prev.filter((_, i) => i !== index))}
        ...
        >
          <X className="w-4 h-4" />
                ...
                <div className="space-y-2">
                  ...
                  <label className="block border-2 border-dashed rounded-lg p-6 text-center cursor-pointer hover:border-blue-500 transition-colors">
  <Upload className="w-8 h-8 mx-auto mb-2 text-gray-400" />
  <p className="text-sm text-gray-600">Cliquez pour ajouter une ou plusieurs attestations</p>
  <p className="text-xs text-gray-400 mt-1">PDF uniquement, max 5MB par fichier</p>
  <input type="file" accept=".pdf" multiple className="hidden" onChange={e => handleFileChange(e, 'certificate')} />
</label>
  ...
  <div className="space-y-2 mt-2">
    {certificateFiles.map((file, index) => (
      <div key={index} className="border-2 border-green-200 bg-green-50 rounded-lg p-3 flex items-center gap-3">
        ...
        <button
          type="button"
          onClick={() => setCertificateFiles(prev => prev.filter((_, i) => i !== index))}
          className="text-red-500 hover:text-red-700"
        >
          <X className="w-4 h-4" />
        </button>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-sm text-blue-900">
                  <strong>À savoir :</strong> Votre candidature sera examinée par notre équipe sous 5 jours ouvrés.
                </p>
              </div>
