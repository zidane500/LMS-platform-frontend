...
export const EditProfile: React.FC = () => {
  ...
  const [photoPreview, setPhotoPreview] = useState<string>(currentUser?.avatar || '');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedUser = await updateProfile({
        ...
        domaines_cibles:  formData.targetDomains.split(',').map(d => d.trim()).filter(Boolean),
  ...
  const roleLabel = currentUser.role === 'learner' ? 'Apprenant'
    : currentUser.role === 'instructor' ? 'Formateur' : 'Administrateur';
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-purple-50/30 dark:from-gray-900 dark:to-purple-900/10">
      <div className="max-w-3xl mx-auto p-6 space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-gray-500 mt-1">Personnalisez vos informations personnelles</p>
        </motion.div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6">
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex gap-2">
                  ...
                  {photoPreview && (
                    <Button type="button" variant="destructive" size="sm" onClick={handleRemovePhoto} className="gap-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <Input value={currentUser.email} readOnly disabled className="bg-gray-100 cursor-not-allowed" />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label className="flex items-center gap-1"><Phone className="w-4 h-4 text-gray-400" /> Téléphone</Label>
                <Input type="tel" value={formData.phone} onChange={e => handleChange('phone', e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label className="flex items-center gap-1"><Calendar className="w-4 h-4 text-gray-400" /> Date de naissance</Label>
                <Input type="date" value={formData.dateOfBirth} onChange={e => handleChange('dateOfBirth', e.target.value)} />
              </div>
            </div>

            <div className="space-y-2">
              <Label className="flex items-center gap-1"><Globe className="w-4 h-4 text-gray-400" /> Langue préférée</Label>
            ...
            <div className="space-y-2">
              <Label className="flex items-center gap-1"><Tag className="w-4 h-4 text-gray-400" /> Domaines d'intérêt</Label>
              <Input value={formData.targetDomains} onChange={e => handleChange('targetDomains', e.target.value)} placeholder="Ex: Développement Web, Data Science" />
              <p className="text-xs text-gray-400">Séparez par des virgules</p>
            </div>

            <div className="space-y-2">
              <Label className="flex items-center gap-1"><Code className="w-4 h-4 text-gray-400" /> Technologies</Label>
              <Input value={formData.technologies} onChange={e => handleChange('technologies', e.target.value)} placeholder="Ex: React, Python, SQL" />
              <p className="text-xs text-gray-400">Séparez par des virgules</p>
            </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 pb-8">
          <motion.div whileHover={{ scale: 1.02 }} className="flex-1">
            <Button onClick={handleSave} disabled={loading} className="w-full h-11 gap-2 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700">
              {loading ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Enregistrement...</>
                       : <><Save className="w-4 h-4" /> Enregistrer les modifications</>}
            </Button>
          </motion.div>
          <Button variant="outline" onClick={() => navigate('/app/profile')} className="h-11 px-6" disabled={loading}>
            Annuler
          </Button>
        </div>
      </div>
