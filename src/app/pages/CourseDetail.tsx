...
import {
  ...
  Play, FileText, Music, Box, Video, Loader2, CheckCircle,
  Lock, BarChart3, Edit, Award,
} from 'lucide-react';
...
export const CourseDetail: React.FC = () => {
  ...
  const [course, setCourse]           = useState<(Course & { isEnrolled?: boolean; statut?: string }) | null>(null);
  const [pageLoading, setPageLoading] = useState(true);
  const [enrolling, setEnrolling]     = useState(false);
  const [isEnrolled, setIsEnrolled]   = useState(false);

  // Contenus
  const [moduleContenus, setModuleContenus]     = useState<Record<string, (Content & { progression?: any })[]>>({});
  const [loadingContenus, setLoadingContenus]   = useState<Record<string, boolean>>({});
  const [openModuleId, setOpenModuleId]         = useState<string | null>(null);
  const [selectedContent, setSelectedContent]   = useState<{ moduleId: string; content: Content & { progression?: any } } | null>(null);
  ...
  const isOwner             = isInstructorOrAdmin && String((course as any).instructorId) === String(currentUser?.id);
  ...
  const getModuleProgress = (moduleId: string): number => {
    ...
    return Math.round((c.filter(x => x.progression?.complete).length / c.length) * 100);
  };
  ...
  const handleSelectContent = (moduleId: string, content: Content & { progression?: any }) => {
    if (!canAccess) {
    ...
    setTimeout(() => document.getElementById('content-viewer')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100);
  };
  ...
  const handleContentComplete = (moduleId: string, contentId: string) => {
    setModuleContenus(prev => ({
      ...
      [moduleId]: (prev[moduleId] || []).map(c =>
        c.id === contentId ? { ...c, progression: { complete: true, pourcentage: 100 } } : c
      ),
    }));
    if (selectedContent?.content.id === contentId) {
      setSelectedContent(prev => prev ? {
        ...prev,
        content: { ...prev.content, progression: { complete: true, pourcentage: 100 } },
      } : null);
    }
  };
  ...
  <div className="relative rounded-3xl overflow-hidden"></div>
            <div className={`${course.thumbnail ? 'hidden' : 'flex'} absolute inset-0 bg-gradient-to-br from-blue-600 via-indigo-700 to-purple-800 items-center justify-center`}>
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              ...
              <span className={`px-3 py-1 rounded-full text-xs font-semibold backdrop-blur-sm border ${
                course.level === 'Débutant'      ? 'bg-green-500/20 text-green-300 border-green-500/30' :
                course.level === 'Intermédiaire' ? 'bg-yellow-500/20 text-yellow-300 border-yellow-500/30' :
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-slate-400 mb-5">
              <span className="flex items-center gap-1.5">
                <Clock className="w-4 h-4 text-blue-400" /> {course.estimatedDuration}h de contenu
              </span>
              <span className="flex items-center gap-1.5">
                <BookOpen className="w-4 h-4 text-blue-400" /> {modules.length} module{modules.length > 1 ? 's' : ''}
              </span>
                      ...
                      <div className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-4 py-2 backdrop-blur-sm">
                        ...
                        <div className="min-w-[100px]">
                          <div className="flex justify-between text-xs text-slate-400 mb-1">
                            ...
                            <span className="text-blue-400 font-semibold">{totalProgress}%</span>
                          </div>
        ...
        {course.description && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
              <h2 className="text-lg font-semibold text-white mb-3">À propos de cette formation</h2>
              <p className="text-slate-400 leading-relaxed">{course.description}</p>
              {course.prerequisites?.length > 0 && (
                <div className="mt-4 pt-4 border-t border-white/10">
                  <p className="text-sm font-medium text-slate-300 mb-2">Prérequis</p>
                  <div className="flex flex-wrap gap-2">
                    {course.prerequisites.map((p, i) => (
                      <span key={i} className="flex items-center gap-1.5 text-xs text-green-400 bg-green-500/10 border border-green-500/20 rounded-lg px-3 py-1">
            ...
            <motion.div id="content-viewer"
              ...
              <div className="bg-gradient-to-br from-blue-500/5 to-indigo-500/5 border border-blue-500/20 rounded-2xl p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-sm font-semibold text-blue-400">Lecture en cours</span>
                  <span className="text-sm text-slate-500">— {selectedContent.content.title}</span>
                </div>
                <ContentViewer
                  ...
                  onComplete={() => handleContentComplete(selectedContent.moduleId, selectedContent.content.id)}
                />
              </div>
            </motion.div>
          )}
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-white/10">
              <h2 className="text-lg font-semibold text-white flex items-center gap-2">
                ...
                <span className="ml-auto text-sm font-normal text-slate-500">{modules.length} module{modules.length > 1 ? 's' : ''}</span>
              </h2>
            </div>

            {modules.length === 0 ? (
              <div className="text-center py-12 text-slate-500">Aucun module disponible</div>
            ) : (
                    ...
                    <div key={module.id}>
                      ...
                      >
                        ...
                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center font-bold text-sm shrink-0 transition-colors ${
                          isOpen ? 'bg-blue-600 text-white' : 'bg-white/10 text-slate-400 group-hover:bg-white/15'
                        }`}>
                          {index + 1}
                        </div>
                        {/* Titre + description */}
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          {isOpen && moduleContenus[module.id] && contenus.length > 0 && currentUser?.role === 'learner' && (
                          <div className="hidden sm:flex items-center gap-2">
                          <div className="w-16 h-1.5 bg-white/10 rounded-full overflow-hidden">
                          <div className="h-full bg-blue-500 rounded-full" style={{ width: `${modProg}%` }} />
                        </div>
                         <span className="text-xs text-slate-500">{modProg}%</span>
                         </div>
                          )}
                          {module.duration > 0 && (
                            <span className="text-xs text-slate-500 hidden sm:block">{module.duration} min</span>
                          )}
                        ...
                        {isOpen && (
                          ...
                          >
                            {isLoadingC ? (
                              ...
                              <p className="text-center text-sm text-slate-600 py-6">Aucun contenu dans ce module</p>
                            ) : (
                              <div className="py-2">
                                {contenus.map((c, ci) => {
                                  const isSelected = selectedContent?.content.id === c.id;
                                  ...
                                  return (
                                    <motion.button
                                      ...
                                      onClick={() => handleSelectContent(module.id, c)}
                                    ...
                                    >
                                      ...
                                      <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 bg-white/5">
                                        {!canAccess ? (
                                          ...
                                          <ContentIcon type={c.type} className="w-3.5 h-3.5" />
                                        )}
                                      </div>
                                      {/* Infos */}
                                      <div className="flex-1 min-w-0">
                                        <p className={`text-sm font-medium truncate ${
                                          isSelected ? 'text-blue-400' : isDone ? 'text-slate-500' : 'text-slate-300'
                                        }`}>
                                          {c.title}
                                        </p>
                                        {c.summary && (
                                          <p className="text-xs text-slate-600 truncate">{c.summary}</p>
                                        )}
                                      </div>
                                      {/* Badges */}
                                      <div className="flex items-center gap-2 shrink-0">
                                        {c.duration > 0 && (
                                          <span className="text-xs text-slate-600">{c.duration} min</span>
                                        )}
                                        {isDone && (
        ...
        {currentUser?.role === 'learner' && !isEnrolled && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
            ...
            <div className="inline-flex flex-col items-center gap-3 px-8 py-6 rounded-2xl bg-gradient-to-br from-blue-500/10 to-indigo-500/10 border border-blue-500/20">
              ...
              <p className="text-white font-semibold">Inscrivez-vous pour accéder aux contenus</p>
              <button
                onClick={handleEnroll}
                disabled={enrolling}
