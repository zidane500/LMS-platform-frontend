...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    if (currentUser && courses.length > 0) {
      const enrolled = courses.filter(
        (course) => userProgress[`${currentUser.id}-${course.id}`],
      );
  ...
  const totalBadges = userProgressData.reduce(
    (sum, p) => sum + p.badges.length,
    0,
  );
  const avgProgress =
    userProgressData.length > 0
      ? Math.round(
          userProgressData.reduce((sum, p) => sum + p.overallProgress, 0) /
            userProgressData.length,
        )
      : 0;
  const totalQuizzes = userProgressData.reduce(
  ...
  const userInstructorRequest = instructorRequests.find(
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                        {userInstructorRequest.status === "pending" &&
                          "Demande de formateur en cours"}
                        {userInstructorRequest.status === "accepted" &&
                          "Demande de formateur acceptée !"}
                        {userInstructorRequest.status === "rejected" &&
                          "Demande de formateur refusée"}
                      </h3>
                      <Badge
                        variant={
                          userInstructorRequest.status === "accepted"
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      <div>
                        ...
                        <span className="font-medium text-gray-900 dark:text-white">
                          {userInstructorRequest.specialty}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-500 dark:text-gray-400">
                        ...
                        <span className="font-medium text-gray-900 dark:text-white">
                          {userInstructorRequest.experience} ans
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-500 dark:text-gray-400">
                        ...
                        <span className="font-medium text-gray-900 dark:text-white">
                          {new Date(
                            userInstructorRequest.createdAt,
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Êtes-vous s��r de vouloir supprimer cette formation ? Cette action
              est irréversible.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
